...
function Login (){
    ...
    const checkData = ()=>{
        if (loginText != data.login) {setLoginValidate(false)}
        if (passText != data.password) {setLoginValidate(false)}
        if (loginText == data.login && passText == data.password){
    ...
    return(
        <div className="login-wrap">
            <div className="login-container">
                <div className={loginValidate? "input-wrap" : "input-wrap alert-validate"} id="login-wrap" data-validate = "Incorect login">
                    <input value = {loginText} onChange = {(e)=>setLoginText(e.target.value)} className="input-item" placeholder="Login" id="login"></input>
                </div>
                <div className={passValidate ? "input-wrap" : "input-wrap alert-validate"} id="pass-wrap" data-validate = "Incorect password">
